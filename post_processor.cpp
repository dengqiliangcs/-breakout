	...
	: PostProcessingShader(shader), Texture(), Width(width), Height(height), Confuse(GL_FALSE), Chaos(GL_FALSE), Shake(GL_FALSE)
{
	...
	glBindRenderbuffer(GL_RENDERBUFFER, this->RBO);//使用了渲染缓冲对象附件
...
void PostProcessor::Render(GLfloat time)//将场景离屏渲染到帧缓冲MSFBO，再将MSFBO作为只读帧缓冲(附件为渲染缓冲对象)，FBO作为只写帧缓冲(附件为纹理)从而得到纹理，最后再渲染到当前屏幕
{
